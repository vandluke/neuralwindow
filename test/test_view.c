...
START_TEST(test_view_create_error)
{
    ...
    for (int64_t i = 0; i < number_of_cases; i++)
    {
        if (!i)
        {
            error = view_create(NULL, offsets[i], ranks[i], shapes[i], strides[i], masks[i]);
        }
        else
        {
...
START_TEST(test_view_create)
{
    ...
    for (int64_t i = 0; i < number_of_cases; i++)
    {
        ...
        ck_assert_ptr_ne(returned_view->mask, masks[i]);

        // Compare shape, mask, and strides with expected
        for (int64_t j = 0; j < ranks[i]; j++)
        {
            ...
            ck_assert_int_eq(returned_view->mask[j], expected_masks[i][j]);
        }
...
START_TEST(test_view_permute)
{
    ...
    for (int64_t i = 0; i < number_of_cases; i++)
    {
        ...
        error = view_create(&original_view, offsets[i], ranks[i], original_shapes[i], original_strides[i], original_masks[i]);
        ck_assert_ptr_null(error);
        error = view_create(&expected_view, offsets[i], ranks[i], expected_shapes[i], expected_strides[i], expected_masks[i]);
        ck_assert_ptr_null(error);
